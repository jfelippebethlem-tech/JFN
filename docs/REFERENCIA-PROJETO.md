...
**Mapa de sinais Massare (ref. StocksToBuyNow.ai — SaaS fechado, só conceito; hype sem backtest):** combinar insider (`FMP.insiderTrades`) + **políticos/congresso** (`FMP.senate`) + 13F (`FMP.form13F`) + earnings/surpresa (`earningsTranscript`+`analyst`) + macro CPI/FOMC (`economics`) + técnicos (`technicalIndicators`) + fundamentos EPS/PE/margem (`statements`). **Acesso FMP (verificado 06-11):** o **MCP do FMP** (sessão Claude) entrega TODOS grátis (senate/insider/13F/macro/news testados OK); a **chave REST grátis** (`FMP_API_KEY` no .env, `massare/fmp.py`) só cobre **fundamentos/ratios/key-metrics/earnings/grades** — insider/senate/13F/macro/técnicos/news são **PAGOS** (402). Logo: serviço autônomo usa a chave p/ fundamentos; sinais diferenciados são **materializados via MCP** no massare.db (refresh depende de mim rodar OU de plano FMP pago). Diferencial honesto do Massare: cada sinal entra **com backtest OOS + proveniência** (≠ hype). LLM gera o "setup explicado".
