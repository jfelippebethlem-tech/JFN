...
export async function renderAcervo(){
  ...
  setTimeout(async () => { const r = await J('/api/acervo/regras?limite=200', {tetoMs: 30000}); const o = await _quandoExistir('ac-regras'); if (o) o.innerHTML = _secRegrasAcervo(r); }, 0);
...
/* Perícia × SIAFE (24/09/2026): a perícia lê o TRECHO; o SIAFE sabe se houve pagamento. 2.082 perícias diziam
   "sem OB no trecho" com OB contabilizada no mesmo processo — a ficha responde sozinha o que ficou em "verificar". */
const _VEREDITO = {
  respondido_pelo_siafe: ['SIAFE responde', 'var(--green)'], pago_bate_liquidado: ['pago = liquidado', 'var(--green)'],
  pago_diverge_liquidado: ['pago ≠ liquidado', 'var(--amber)'], sem_ob_nas_duas_fontes: ['sem OB nas duas fontes', 'var(--rose)'],
};
function _secReconciliacao(f){
  const r = f.reconciliacao || [];
  if (!r.length) return '';
  return card(`<div style="font-weight:700">Perícia × SIAFE</div>`
    + r.map(x => { const [rotulo, cor] = _VEREDITO[x.veredito] || [x.veredito, null];
      return `<div style="margin:8px 0">${tag(rotulo, cor)} ${esc(x.diz)}</div>`; }).join('')
    + `<div class="dim">A perícia lê o trecho do processo; o SIAFE é a fonte do pagamento (só OB Contabilizada). Nenhuma das duas, sozinha, conclui irregularidade.</div>`);
}

/* Duas camadas (25/09/2026): a REGRA (cronologia_do_ato — datas escritas dentro das peças, determinística)
   e, por cima, a LEITURA do analista (Claude leu os autos inteiros): o que a regra não vê, com parecer. */
const _GR = {vermelho: ['🔴', 'var(--rose)'], amarelo: ['🟡', 'var(--amber)'], verde: ['🟢', 'var(--green)']};
function _secCronologia(f){
  const c = f.cronologia;
  if (!c || c.grau === 'nao_aplicavel') return '';
  const [ic, cor] = _GR[c.grau] || ['', null];
  return sec('Cronologia dos atos — regra') + card(`<div style="font-weight:700">${ic} ${tag(c.grau, cor)} datas lidas dentro das peças</div>`
    + ((c.achados || []).map(a => `<div style="margin:8px 0">${tag((_GR[a.grau] || [''])[0] + ' ' + a.tipo.replace(/_/g, ' '), (_GR[a.grau] || [])[1])} ${esc(a.diz)}<div class="dim">${esc(a.fundamento || '')}</div></div>`).join('')
...
/* Replicação (25/09/2026): as regras que a leitura processo a processo ensinou, rodadas no acervo inteiro. */
function _secRegrasAcervo(r){
  ...
  return sec('Regras acesas no acervo', r.total) + card(`<div class="dim">${fmtN(r.processos_lidos)} processos lidos por regra em ${esc(fmtData(r.gerado_em))} · ${pr}</div>`
    + `<div style="overflow-x:auto;margin-top:8px"><table class="tb"><thead><tr><th>processo</th><th>regra</th><th style="text-align:left">o que diz</th><th>parecer</th></tr></thead><tbody>`
    + r.itens.map(it => it.regras.map((g, i) => `<tr>${i ? '<td></td>' : `<td><a href="#" data-acervo="abrir" data-numero="${esc(it.numero)}"><b>${esc(it.numero)}</b></a></td>`}
        <td>${tag((_GR[g.grau] || [''])[0] + ' ' + g.regra.split(':')[1].replace(/_/g, ' '), (_GR[g.grau] || [])[1])}</td>
        <td style="text-align:left" class="dim">${esc(g.diz.slice(0, 260))}</td><td>${i ? '' : (it.lido_pelo_analista ? '✔ lido' : '—')}</td></tr>`).join('')).join('')
    + `</tbody></table></div>`);
}
function _secFamilia(f){
  ...
  return sec('Processos ligados') + card(`<div class="dim">Contratação, pagamentos e aditivos se explicam juntos: a leitura íntegra passa por estes (página coletiva do D.O. fora).</div>`
    + tab('Este processo cita', fa.cita || []) + tab('É citado por', fa.citado_por || []));
}
function _secLeitura(f){
  ...
  return sec('Leitura do analista — parecer') + card(`<div style="font-weight:700">${ic} ${tag(l.grau, cor)} ${esc(l.leitor || '')}</div>`
    + `<p style="margin:8px 0"><b>Parecer:</b> ${esc(l.parecer || '')}</p>`
    + ((l.linha_do_tempo || []).length ? `<div style="overflow-x:auto"><table class="tb"><thead><tr><th>quando</th><th style="text-align:left">ato</th></tr></thead><tbody>`
       + l.linha_do_tempo.map(([q, a]) => `<tr><td class="dim" style="white-space:nowrap">${esc(q)}</td><td style="text-align:left">${esc(a)}</td></tr>`).join('') + `</tbody></table></div>` : '')
    + (l.achados || []).map(a => `<div style="margin:6px 0">${(_GR[a.grau] || [''])[0]} ${esc(a.diz)} <span class="dim">— ${esc(a.fundamento || '')}</span></div>`).join('')
    + ((l.conexoes || []).length ? `<div style="margin-top:8px"><b>Conexões</b><ul>${lista(l.conexoes)}</ul></div>` : '')
    + ((l.verificar || []).length ? `<div><b>A verificar</b><ul>${lista(l.verificar)}</ul></div>` : '')
    + `<div class="dim">${esc(l.ressalvas || '')}${l.caso ? ' · caso: ' + esc(l.caso) : ''}</div>`);
}

function _secPericias(f){
  const p = f.pericias || {}; let h = _secReconciliacao(f);
