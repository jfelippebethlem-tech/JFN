...
export async function renderTacRecorrente(){
  ...
  h+=`<div style="overflow-x:auto"><table class="tb"><thead><tr><th>fornecedor</th><th class="right">TACs</th><th class="right">soma publicada</th><th class="right">órgãos</th><th class="right">processos</th><th>período</th></tr></thead><tbody>`
    +(d.itens||[]).map(x=>`<tr><td>${esc(x.fornecedor)}</td><td class="right"><b>${fmtN(x.n)}</b></td><td class="right">${fmtRc(x.soma||0)}${x.n_com_valor<x.n?` <span class="dim">(${fmtN(x.n_com_valor)} c/ valor)</span>`:''}</td><td class="right">${fmtN(x.n_orgaos)}</td><td class="right">${fmtN(x.processos)}</td><td class="dim">${esc(x.de)} → ${esc(x.ate)}</td></tr>`).join('')
    +`</tbody></table></div>`;
