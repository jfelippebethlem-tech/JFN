...
export async function fazBusca(){
  ...
  /* DOERJ: a API devolve data · tipo · órgão · título + trecho com <b> do FTS. Antes o card
     despejava JSON.stringify(x) — desde 09/09/2026 o corpus é o PDF integral e merece leitura. */
  const _doeCard=x=>{const tr=esc(x.excerpt||x.texto||'').replace(/&lt;b&gt;/g,'<b>').replace(/&lt;\/b&gt;/g,'</b>');
...
/* ── TAC recorrente (DOERJ) ───────────────────────────────────────────────────────────────────────
   Termo de Ajuste de Contas paga serviço prestado SEM contrato (Decreto 47.283/2020). Um TAC é exceção;
   67 TACs em três meses para o mesmo fornecedor é o modo de pagar. Corpus = PDF integral do DOERJ
   (desde 09/09/2026); tabela doerj_tac refeita toda noite pelo cruzador. */
