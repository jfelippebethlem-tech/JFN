...
async function zerosSemCausa(){
  ...
  h+=`<div class="grid g2">${kpi(fmtRc(d.valor_ob_sem_causa),'OB atrás dos vazios','var(--rose)','💸',
      ...
      kpi(fmtN(d.zeros),'Zerados no total',null,'📄',
      {sobre:'De '+fmtN(d.processos_com_registro)+' processos com registro de leitura. A maioria tem causa conhecida (restrito, inexistente, ou já capturado por outro caminho); o card mostra os que não têm.'})}</div>`;
  ...
  h+=card(`<table class="tb"><thead><tr><th>processo</th><th class="right">OB paga</th></tr></thead><tbody>`
    +d.itens.map(x=>`<tr><td>${esc(x.processo)}</td><td class="right">${fmtRc(x.valor_ob)}</td></tr>`).join('')
