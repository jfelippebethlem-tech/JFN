...
📄 <a href="${alvo.url}" target="_blank" style="color:var(--accent)">abrir PDF</a> (apaga ao sair)`;
  ...
  function _intDelegar(ev) {
    ...
    if (!el) return;
    const [acao2, arg] = (el.dataset.int || "").split(":");
    if (acao2 === "buscar") {
      const i = document.getElementById("intq");
      _intQ = i ? i.value : "";
    } else if (acao2 === "fonte") {
      _intFonte = _intFonte === arg ? "" : arg;
    } else if (acao2 === "ordem") {
      _intOrdem = arg;
    } else if (acao2 === "abrir") {
  ...
  if (typeof document !== "undefined" && !document.__intLigado) {
    ...
    document.addEventListener("keydown", (ev) => {
      if (ev.key === "Enter" && ev.target && ev.target.id === "intq") {
        _intQ = ev.target.value;
        _intIr();
      }
    });
  }
  async function renderIntegras() {
    ...
    let h = cover(
      "prefeitura",
      "Íntegras · Contratos do Município do Rio",
      "O documento assinado de cada contrato, capturado do PNCP. Clique para ler a íntegra, baixar o texto ou abrir o original.",
      "doc"
    );
    h += `<div class="grid">
  ...
  async function renderTacRecorrente() {
    ...
    ${kpi(fmtN(d.sinais_vermelhos || 0) + " / " + fmtN(d.sinais_total || 0), "Sinais 🔴 / total", "var(--rose)", null, { sobre: "Cruzamentos sobre os fornecedores com 3+ TACs: agente público no QSA (🔴 se da saúde estadual), sócio comum entre fornecedores de TAC, capital ínfimo, fachada, emergência recorrente, doação eleitoral. Casamento de agente por NOME — homônimo exige CPF. Sinal é indício, nunca acusação." })}
    ${kpi(fmtN((d.por_sinal || {}).contrato_vencido_tac || 0), "Contrato venceu → TAC", ((d.por_sinal || {}).contrato_vencido_tac || 0) > 0 ? "var(--rose)" : null, null, { sobre: "Fornecedores cujo período indenizado por TAC começa até 45 dias depois do fim de vigência de um contrato do PRÓPRIO fornecedor no registro do TCE-RJ — o serviço seguiu por indenização em vez de nova licitação (medido 10/09: TUISE, HETO Melchiades: contrato até 13/01/2026, TAC desde 14/01/2026). 🔴 quando nenhum contrato vigia na data." })}
    ${kpi(fmtN((d.por_sinal || {}).emergencia_siga || 0), "Emergência como regime (SIGA)", ((d.por_sinal || {}).emergencia_siga || 0) > 0 ? "var(--rose)" : null, null, { sobre: 'Fornecedores de TAC com 3+ contratações "Dispensa - Especial" (emergência, art. 24 IV / art. 75 VIII) no Portal SIGA — registro público de contratos do Estado, lido sem API. 🔴 quando 10+ e metade ou mais das contratações são emergenciais (TUISE: 31 de 42).' })}
    ${kpi(fmtN((d.por_sinal || {}).previsto_no_pca || 0), "Estava no plano (PCA)", null, null, { sobre: "Fornecedores cujo objeto do TAC consta no Plano de Contratações Anual da unidade (extração pública do SIGA): o serviço foi planejado, orçado e pago por indenização sem licitação — a lacuna não é imprevisível (art. 12, VII e art. 75, §6º Lei 14.133)." })}
    ${kpi(fmtN((d.por_sinal || {}).sancao_siga || 0), "Sanção estadual vigente", ((d.por_sinal || {}).sancao_siga || 0) > 0 ? "var(--rose)" : null, null, { sobre: "Fornecedores de TAC com sanção VIGENTE no registro estadual do SIGA (3.026 sanções, 338 vigentes). 🔴 quando o enquadramento veda contratar (inidoneidade/impedimento); advertência e multa não vedam." })}
  ...
  async function laiGerar() {
    const alvo = ($("lai-alvo")?.value || "").trim();
    const o = $("lai-out");
    if (!alvo) {
      o.innerHTML = card('<div class="warn">Informe o alvo.</div>');
      return;
    ...
    try {
      ...
      const tb = document.querySelector("table.tb tbody");
      if (tb) {
        tb.insertAdjacentHTML("afterbegin", _linha({ ...d, status: "rascunho", vencido: false }));
      }
    } catch (e) {
      o.innerHTML = card(`<div class="warn">${erroHumano(String(e))}</div>`);
    }
  ...
  async function laiStatus(id, status) {
    ...
    if (status === "protocolado") {
      protocolo = window.prompt("Nº do protocolo no e-SIC (opcional):", "") || null;
    }
    try {
      const d = await J("/api/lai/status", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, status, protocolo }) });
      if (!d || !d.ok) {
        alert(erroHumano((d || {}).erro || "falhou"));
        return;
      }
      const it = d.item || {};
      const cel = [...document.querySelectorAll("table.tb tbody tr")].find((tr) => tr.firstElementChild && tr.firstElementChild.textContent === `#${id}`);
      if (cel) {
        cel.outerHTML = _linha({ ...it, vencido: false });
      }
    } catch (e) {
      alert(erroHumano(String(e)));
    }
  }
  function ligarLai() {
    document.addEventListener("click", (ev) => {
  ...
  var GRAU = (g) => {
    ...
    if (/EXTREMO|ALTO|🔴|CRIT/.test(s)) return "var(--rose)";
    if (/MEDIO|MÉDIO|🟡|ATEN/.test(s)) return "var(--amber)";
    return null;
  };
  ...
  async function acervoBuscar(q, esf) {
    ...
    const it = r.hits || [];
    if (!it.length) {
      o.innerHTML = card('<div class="dim">Nada encontrado nas bases da casa. Nº de processo desconhecido pode ser capturado: Estado → sei_fila_captura; Prefeitura → busca livre no SEI municipal.</div>');
      return;
    }
    ...
    <div style="overflow-x:auto"><table class="tb"><thead><tr><th>esfera</th><th>processo</th><th>objeto / tipo</th><th>órgão / fornecedor</th><th>grau</th><th>pago</th><th>docs</th><th>fontes</th></tr></thead><tbody>` + it.map((h) => `<tr><td>${esc(h.esfera)}</td><td><a href="#" data-acervo="abrir" data-numero="${esc(h.numero)}"><b>${esc(h.numero)}</b></a></td>
      ...
      <td class="num">${h.total_pago != null ? fmtRc(h.total_pago) : "—"}</td><td>${h.n_docs != null ? fmtN(h.n_docs) : "—"}</td><td class="dim">${esc(h.fonte || "")}</td></tr>`).join("") + `</tbody></table></div>`;
  ...
  function _secDocs(f) {
    ...
    <div style="overflow-x:auto;max-height:420px;overflow-y:auto"><table class="tb"><thead><tr><th>#</th><th>documento</th><th>tipo / fase</th><th>unidade / data</th><th>chars</th><th></th></tr></thead><tbody>` + docs.map((d) => `<tr><td>${esc(d.seq)}</td><td>${esc((d.titulo || "").slice(0, 100))}</td><td class="dim">${esc(d.tipo || "")}${d.fase ? " · " + esc(d.fase) : ""}</td>
      <td class="dim">${esc(d.unidade || "")}${d.data ? " · " + esc(d.data) : ""}</td><td class="num">${d.chars ? fmtN(d.chars) : "—"}${d.ocr ? ' <span class="tag">OCR</span>' : ""}</td>
      <td>${d.texto_path || d.com_texto ? `<button type="button" class="btn ghost" data-acervo="ler" data-numero="${esc(f.numero)}" data-seq="${esc(d.seq)}">ler</button>` : d.url ? `<a class="dim" href="${esc(d.url)}" target="_blank" rel="noopener">fonte</a>` : '<span class="dim">só listado</span>'}</td></tr>`).join("") + `</tbody></table></div><div id="ac-doc" style="margin-top:8px"></div>`;
  }
  function _secAchados(f) {
    const a = f.achados || [];
    let h = "";
    if (a.length) h += `<div style="overflow-x:auto"><table class="tb"><thead><tr><th>fonte</th><th>código</th><th>grau</th><th>o que diz</th><th>apoio</th></tr></thead><tbody>` + a.map((x) => `<tr><td class="dim">${esc(x.fonte)}</td><td><b>${esc(x.codigo || "")}</b></td><td>${tag(x.grau, GRAU(x.grau))}${x.score != null ? " " + esc(x.score) : ""}</td><td>${esc((x.diz || "").slice(0, 220))}</td><td class="dim">${esc((x.apoio || "").slice(0, 160))}</td></tr>`).join("") + `</tbody></table></div>`;
  ...
  async function acervoAbrir(numero) {
    const o = $("ac-ficha");
    if (!o) return;
  ...
  var TABS = {
    ...
    geral: [
      ...
      { id: "g_acervo", ic: "🗂️", tl: "Acervo", render: renderAcervo },
