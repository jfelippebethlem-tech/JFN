(() => {
  ...
  async function missaoCriar() {
    ...
    jfnToast("Missão enfileirada.", "green");
    $("ms-txt").value = "";
  ...
  addEventListener("hashchange", () => {
    ...
    if (!id) return;
    if (!_abaValida(id)) {
      ir("i_cockpit").then(() => _hashInvalido(id));
      return;
    }
    if (id !== aba) ir(id);
  });
  ...
  async function seiBaixarZip(cnpj) {
    ...
    if (!r.ok) {
      jfnToast("Não consegui montar o ZIP agora — " + (r.erro || "tente de novo em instantes."), "rose");
      return;
    }
    window.open(r.url, "_blank");
  }
...
` + (dd.socios || []).slice(0, 6).map((s) => "• " + (s.nome || s)).join("\n");
  ...
  async function sweep(a) {
    ...
    jfnToast(a === "pausar" ? "Sweeps pausados." : "Sweeps retomados.", "green");
...
📄 <a href="${alvo.url}" target="_blank" style="color:var(--accent)">abrir PDF</a> (apaga ao sair)`;
  ...
  addEventListener("pointermove", (e) => {
    _ckMX = e.clientX / innerWidth;
    _ckMY = e.clientY / innerHeight;
  }, { passive: true });
  var _rjbgTinge = () => {
  };
  function rjbgStart() {
...
void main(){
  ...
  (() => {
    const cx = {
      ...
      _compDisp: [() => _compDisp, (v) => {
        _compDisp = v;
      }],
      _compEsf: [() => _compEsf, (v) => {
        _compEsf = v;
      }],
      _compGrupo: [() => _compGrupo, (v) => {
        _compGrupo = v;
      }],
      _compOrd: [() => _compOrd, (v) => {
        _compOrd = v;
      }],
      _compTermo: [() => _compTermo, (v) => {
        _compTermo = v;
      }],
