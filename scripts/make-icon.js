...
<svg xmlns="http://www.w3.org/2000/svg" width="${S}" height="${S}" viewBox="0 0 ${S} ${S}">
  <defs>
    ...
    <filter id="soft" x="-30%" y="-30%" width="160%" height="160%">
      <feDropShadow dx="0" dy="10" stdDeviation="14" flood-color="#0b1026" flood-opacity="0.35"/>
    </filter>
  </defs>
...
sharp(Buffer.from(svg))
  ...
  .then(() => sharp(Buffer.from(svg)).resize(512, 512).png().toFile(path.join(outDir, 'icon-512.png')))
  .then(() => console.log('Ícones gerados em public/brand/'))
