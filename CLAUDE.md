...
- 🧭 **NÃO REINVENTAR (gatilhos; detalhe no vault):** (1) **OB/pagamento → SEMPRE SIAFE direto, nunca o espelho TFE** (`siafe_ob_orcamentaria --por-ug`/`coletar_obs_sessao`); (2) **relatório/dossiê = produto da casa** `reporting/inteligencia.py` + `render_html`/`html_to_pdf` (Kroll, PDF) — nunca .txt à mão; (3) **processo SEI = ARQUIVO primeiro** — `tools/sei_consultar.py` (texto+fases+fotos de medição em `data/sei_arquivo/`, grátis) antes de browser/IA; caminho único `docs/PLAYBOOK-SEI.md`; (4) **duplicidade de contrato contínuo = lente de COMPETÊNCIA** (não valor): `compliance_agent/duplicidade_competencia.py` (guards: lag, dez lag-0, split=mesmo RE, reajuste-complemento, renovação ≠ ano civil) — só a NF fecha; (5) **vício de licitação → catálogo canônico primeiro** `knowledge/catalogo_vicios.py` (48 vícios — 45 cobertos, 2 parciais, 1 lacuna; `lacunas()` declaradas) + graus de flag `editais/flags.grau_flag` (LLM nunca produz flag CERTO) + escalada `editais/escalada.recomendar()` — plano-mestre `docs/superpowers/plans/2026-07-20-dossie-mestre.md`; (6) **pacote completo por CNPJ = `/api/dossie/completo`** (`dossie.montar_ctx_completo` = 360 + fachada + cláusulas na íntegra + suspeitas + árvore SEI + Lex + planilha); TODO deliverable passa pelo **gate `reporting/neutralidade.garantir_neutro`** (zero JFN/Lex/paths — teste trava goldens); **enxame (`editais_direcionamento --so-rj`) só OFF-HOURS** — escrever no compliance.db TRAVA as rotas de leitura do painel. Síntese reflexiva: `docs/SINTESE-REFLEXIVA-2026-07-20.md`. Notas: `~/vault/codigo/relatorio-pipeline.md` · `~/vault/aprendizados/{fonte-ob-sempre-siafe-nunca-tfe,duplicidade-ob-competencia-vs-valor,loops-dado-real-dossie-mestre}.md` · `~/vault/casos/iterj-mgs-clean-pagamentos.md`.
...
This project is indexed by GitNexus as **JFN** (38500 symbols, 60335 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
