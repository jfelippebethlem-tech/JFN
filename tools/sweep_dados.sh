...
# resumível (pula quem já tem visual_classe). 90/ciclo cabe no time-bound; o sweep_sede grava no MESMO DB → busy_timeout.
if [ -f data/.pause_fachada_visual_sweep ]; then say "fachada visual pausada — pulei"; else $PRIO timeout 900 $PY -m tools.fachada_visual_sweep --status INDICIO --limite 90 --max-min 13 --pausa 0.3 >> data/fachada_visual.log 2>&1; say "fachada visual rc=$?"; fi
say "fim"
