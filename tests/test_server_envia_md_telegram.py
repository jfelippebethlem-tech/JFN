...
from rotas import produtos as _produtos  # noqa: E402 — split 2026-07-06: helper migrou de server.py
...
def test_inclui_md_junto_de_pdf_xlsx_lex(monkeypatch):
    ...
    asyncio.run(_produtos._enviar_docs_telegram(result, "Relatório X"))
...
def test_pdf_vem_antes_do_md(monkeypatch):
    ...
    asyncio.run(_produtos._enviar_docs_telegram(result, "T"))
...
def test_sem_arquivos_avisa(monkeypatch):
    enviados, msgs = _capturar(monkeypatch)
    asyncio.run(_produtos._enviar_docs_telegram({"resumo": ""}, "T"))
    assert enviados == []
